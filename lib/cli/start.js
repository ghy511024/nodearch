...
const tsNodeIndexPath = fileURLToPath(import.meta.resolve('ts-node'));
const tsNodeDirPath = path.join(path.dirname(tsNodeIndexPath), '..');
const tsNodeESMLoaderPath = path.join(tsNodeDirPath, 'esm.mjs');
const tsNodeESMLoaderPathURL = pathToFileURL(tsNodeESMLoaderPath);
...
try {
  ...
  const { data: latestPkgInfo } = await axios('https://registry.npmjs.org/@ghy_test_nodearch/cli/latest', { timeout: 1000 });
  ...
  if (latestPkgInfo.version !== currentCliVersion) {
    ...
    print(cliColors.yellow, `Run npm i -g @ghy_test_nodearch/cli to update`);
    console.log();
  }
}
catch (err) { }
...
if (getLoadMode() === 'ts') {
  spawn('node', ['--no-warnings=ExperimentalWarning', '--experimental-specifier-resolution=node', '--loader', tsNodeESMLoaderPathURL, appStart, ...appArgs], {
    stdio: 'inherit'
  });
}
